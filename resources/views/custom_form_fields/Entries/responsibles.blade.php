    ...
    @if (class_exists($options->model))
        ...
        @if ($options->type == 'belongsTo')
            ...
            @if (isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @endif
            @else
                <select name="responsible_id" class="select2" id="responsibles_select"></select>

                @push('javascript')
                    <script>
                        $("select[name='client_id']").on("change", () => {
                            $("#responsibles_select").empty();
                            $.post({
                                url: "{{ route('getResponsiblesByClient') }}",
                                data: {
                                    client_id: $("select[name='client_id']").val()
                                },
                                success: (result) => {
                                    result.forEach(responsible => {
                                        let option =
                                            `<option value="${responsible.id}">${responsible.name} ${responsible.last_name}</option>`
                                        $("#responsibles_select").append(option)
                                    });
                                }
                            });
                        })

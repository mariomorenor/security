    ...
    @if (class_exists($options->model))
        ...
        @if ($options->type == 'belongsTo')
            ...
            @if (isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @endif
            @else
                <select name="camera_id" id="cameras_select" class="select2"></select>

                @push('javascript')
                    <script>
                        $("select[name='client_id']").on("change", () => {
                            $("#cameras_select").empty();
                            $.post({
                                url: "{{ route('getCamerasByClient') }}",
                                data: {
                                    client_id: $("select[name='client_id']").val()
                                },
                                success: (result) => {
                                    result.forEach(camara => {
                                        let option = `<option value="${camara.id}">${camara.name}</option>`
                                        $("#cameras_select").append(option)
                                    });
                                },
                                error: () => {
                                    console.log("poi");
